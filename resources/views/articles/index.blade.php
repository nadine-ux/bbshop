...
<div class="search-section">
    <form method="GET" action="{{ route('articles.index') }}" id="filterForm">
        ...
        <div class="filters-row">
            ...
            <div class="filter-item">
                <label><i class="fas fa-warehouse"></i> État stock</label>
                <select name="etat_stock">
                    <option value="">Tous</option>
                    <option value="normal"  {{ request('etat_stock')=='normal'  ? 'selected' : '' }}>✅ Normal</option>
                    <option value="faible"  {{ request('etat_stock')=='faible'  ? 'selected' : '' }}>⚠️ Faible</option>
...
.camera-box {
    ...
    border: 2px solid var(--green); background: #000;
}
.camera-box__header {
    display: flex; justify-content: space-between; align-items: center;
    padding: .55rem 1rem; background: var(--green); color: white;
...
@keyframes scanLine { 0%{top:40px} 100%{top:200px} }
.camera-hint {
...
#cameraStatus { padding: .4rem 1rem; background: #111; color: #aaa; font-size: .78rem; }
...
.filter-item select {
    ...
    transition: border-color .25s; cursor: pointer; max-width: 140px;
}
...
.card-img-placeholder {
    ...
    justify-content: center; font-size: 3.5rem; color: #ddd;
}
.stock-badge {
    position: absolute; top: 9px; left: 9px;
    font-size: .7rem; font-weight: 700; padding: .22rem .6rem;
    border-radius: 20px; backdrop-filter: blur(4px); box-shadow: 0 2px 8px rgba(0,0,0,.2);
}
...
.card-img-hover {
    ...
    justify-content: center; gap: .4rem; transition: background .3s;
}
.article-card:hover .card-img-hover { background: rgba(0,0,0,.38); }
.card-img-hover i, .card-img-hover span {
    color: white; opacity: 0; transform: translateY(6px); transition: all .3s;
}
.card-img-hover i    { font-size: 1.6rem; }
.card-img-hover span { font-size: .85rem; font-weight: 700; transition-delay: .05s; }
...
.card-meta { display: block; font-size: .78rem; color: var(--muted); margin-bottom: .2rem; }
...
/* Actions */
.card-actions { display: flex; border-top: 2px solid var(--bg); background: #fafafa; }
.card-btn {
...
.card-btn--history:hover { background: #f3e5f5; }
...
.card-btn--delete:hover { background: #fdecea; }

/* Empty */
.empty-state {
    grid-column: 1/-1; text-align: center; padding: 3.5rem 2rem;
    background: white; border-radius: var(--radius); box-shadow: var(--shadow); color: var(--muted);
}
...
.popup-loading {
    display: flex; flex-direction: column; align-items: center;
    justify-content: center; padding: 3rem; color: var(--muted); gap: 1rem;
}
.spinner {
...
/* Popup détail */
.detail-img { width: 100%; height: 220px; object-fit: cover; border-radius: 20px 20px 0 0; display: block; }
.detail-img-ph {
    width: 100%; height: 160px; background: var(--bg);
    border-radius: 20px 20px 0 0; display: flex; align-items: center;
    justify-content: center; font-size: 5rem; color: #ddd;
}
...
.detail-barcodes { margin-bottom: 1rem; }
.detail-barcodes .bc-tag {
    display: inline-flex; align-items: center; gap: .35rem;
    background: var(--bg); border: 1.5px solid var(--border);
    border-radius: 8px; padding: .3rem .7rem; font-size: .82rem;
    font-weight: 600; color: var(--text); margin: .2rem .2rem .2rem 0;
}
.detail-barcodes .bc-tag.primary { border-color: var(--green); background: #f0fdf4; color: var(--green); }
.detail-barcodes .bc-tag i { font-size: .75rem; }
...
.history-tabs { display: flex; gap: .5rem; padding: 1rem 1.5rem; border-bottom: 2px solid var(--border); }
.htab {
...
.htab[data-filter="entree"].active { background: var(--green); border-color: var(--green); }
.htab[data-filter="sortie"].active { background: var(--red);   border-color: var(--red); }
...
.mv-icon { width: 38px; height: 38px; border-radius: 10px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: .95rem; }
...
function openDetail(id) {
    ...
    fetch(`/articles/${id}/detail-json`)
        ...
        .then(a => {
            ...
            // Afficher tous les codes-barres si disponibles
            let barcodesHtml = '';
            if (a.barcodes && a.barcodes.length > 0) {
                barcodesHtml = `<div class="detail-barcodes">` +
                    a.barcodes.map(b => `
                        <span class="bc-tag ${b.is_primary ? 'primary' : ''}">
                            <i class="fas fa-barcode"></i>
                            ${b.code_barres}
                            ${b.label ? `<em style="opacity:.7">(${b.label})</em>` : ''}
                            ${b.is_primary ? '<i class="fas fa-star" style="font-size:.65rem"></i>' : ''}
                        </span>`).join('') +
                    `</div>`;
            } else if (a.code_barres) {
                barcodesHtml = `<p class="detail-code"><i class="fas fa-barcode"></i> ${a.code_barres}</p>`;
            }
                ...
                <div class="detail-body">
                    ...
                    <div class="detail-grid">
                        <div class="detail-cell">
                            ...
                            <div class="detail-cell__val">${a.stock} pcs<br>
                                <small style="font-weight:500;color:#999">${cartons} cartons + ${reste} pcs</small>
                            </div>
                        </div>
                        <div class="detail-cell">
...
function openHistory(id, nom) {
    ...
    fetch(`/articles/${id}/mouvements-json`)
        .then(r => r.json())
        .then(data => { allMouvements = data; renderHistory('all'); })
        .catch(() => {
            document.getElementById('historyBody').innerHTML =
        ...
        <div class="mv-item">
            ...
            <div class="mv-info">
                ...
                <div class="mv-qty">${m.type === 'entree' ? '+' : '-'}${m.quantite} pcs
                    ${m.cartons != null ? `<small style="font-weight:500;color:#999">(${m.cartons} ctn + ${m.pieces} pcs)</small>` : ''}
                </div>
