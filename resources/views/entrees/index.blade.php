...
<div class="inv-wrap">
    ...
    <div class="inv-topbar">
        <div>
            <h4 class="inv-title">Liste des entrées</h4>
            <small class="text-muted">Gestion des bons de réception</small>
        </div>
        <a href="{{ route('entrees.create') }}" class="btn-inv-primary">
    ...
    <div class="inv-card mb-3">
        <form method="GET" action="{{ route('entrees.index') }}" class="filters-row">

            <div class="field">
                <label><i class="fas fa-building fa-xs"></i> Fournisseur</label>
            ...
            <div class="field">
                ...
                <select name="sort">
                    ...
                    <option value="fournisseur"    {{ request('sort')==='fournisseur' ? 'selected':'' }}>Fournisseur</option>
                </select>
            </div>
            ...
            <div class="field" style="max-width:120px">
                ...
                <div style="display:flex;gap:6px">
                    ...
                    <a href="{{ route('entrees.index') }}" class="btn-reset">
...
<div class="inv-card">
    <div class="table-responsive">
        <table class="inv-table">
            ...
            @forelse($entrees as $entree)
                <tr>
                    <td>
                        <span class="bon-num">#{{ $entree->id }}</span>
                    </td>
                    ...
                    <td>
                        @if($entree->fournisseur)
                            <div style="display:flex;align-items:center;gap:8px">
                                ...
                                <span style="font-weight:500;font-size:13px">{{ $entree->fournisseur->nom }}</span>
                            </div>
                        @else
                    ...
                    <td style="text-align:right">
                        ...
                        @endphp
                        <strong style="font-size:13px">{{ number_format($montantTotal, 2) }} DA</strong>
                    </td>

                    <td style="text-align:center">
                        <div style="display:flex;justify-content:center;gap:5px">
                            <a href="{{ route('entrees.show', $entree) }}" class="action-btn action-view" title="Voir">
                                <i class="fas fa-eye" style="font-size:11px"></i>
                            </a>
                            <a href="{{ route('entrees.edit', $entree) }}" class="action-btn action-edit" title="Modifier">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:.75rem 1.25rem;border-top:0.5px solid #f0f0f0">
        <small class="text-muted">Page {{ $entrees->currentPage() }} sur {{ $entrees->lastPage() }} — {{ $entrees->total() }} résultats</small>
        {{ $entrees->links() }}
    </div>
    @endif
</div>
...
.badge-articles:hover { background: #C0DD97; }
